...
const inboundStateMachine = require('./inbound-state-machine')
const outboundStateMachine = require('./outbound-state-machine')
const IDENTITY_TYPE = 'identity'
const sleep = async (milliseconds) =>
  new Promise((resolve) => setTimeout(() => resolve(), milliseconds))
const createConnectionStateMachine = (connection, machineDefinition) => {
  const { addMethod, handleEvent, label } = createStateMachine({
...
const newConnection = ({ socket, emitter, fromIdentity, serverUrl }) => {
  ...
  const label = stateMachine.label
  const checkConnectivity = (timeout) => {
    return new Promise((resolve) => {
      const pingId = Date.now()
      const pongReceived = (buffer) => {
        const receivedPingId = buffer
          ? parseInt(buffer.toString(), 10)
          : 0
      ...
      return new Promise((resolve, reject) => {
        ...
        webSocketServer.on('error', async (error) => {
          ...
          if (error.code === 'EADDRINUSE' && timeRemaining > 0) {
            await sleep(
              Math.max(10, Math.min(timeRemaining, retryInterval))
            )
